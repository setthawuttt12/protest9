    ...
    <v-container>
        <v-row>
            <v-col cols="12">
                ...
                <v-alert type="info" v-else-if="user.status_eva === 2">รอกรรมการประเมิน</v-alert>
                <v-alert type="error" v-else-if="user.status_eva === 1">ยังไม้ได้ประเมินตนะเอง</v-alert>
...
onMounted(async () =>{
    await Promise.all([fetchUser(),fetchTopics(),fetchCommit(),fetchScore()])
})
